...
class AppStore extends Component {
  ...
  render() {
    const {searchInput, tab} = this.state
    const tablist = appsList.filter(each => each.category === tab)
    const filteredList = tablist.filter(each =>
      each.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return (
      <div className="bg-cont">
        <div className="cont">
          <h1 className="hed">App Store</h1>
          ...
          <ul className="tab-cont">
            {tabsList.map(eachTable => (
              <TabItem
                ...
                key={eachTable.tabId}
                isActive={eachTable.tabId === tab}
              />
            ))}
          </ul>
          <ul className="app-cont">
            {filteredList.map(eachMap => (
              <AppItem details={eachMap} key={eachMap.appId} />
            ))}
          </ul>
        </div>
      </div>
